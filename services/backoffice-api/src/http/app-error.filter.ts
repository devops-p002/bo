...
const logger = createLogger('backoffice-api-http');

/** Converts every thrown error into an RFC 7807 problem-details
 * response - see platform/CLAUDE.md's "Every service's AppErrorFilter
 * must handle NestJS's own HttpException first" for why the
 * HttpException branch runs before toProblemDetails. */
@Catch()
